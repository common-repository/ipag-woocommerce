...
abstract class WC_iPag_Loader extends WC_Payment_Gateway
{
    ...
    protected static function getProp($order, $prop)
    {
        if ($order) {
            ...
            else if (property_exists($order, $prop))
                return $order->$prop;
        }
...
?>
    ...
    public static function callback_handler()
    {
        ...
        if (
            ...
            if (in_array($xml->status_pagamento, [5, 8])) { // Aprovado
                ...
                // WC_Subscriptions_Manager::process_subscription_payment_failure_on_order($parentOrderId);
    ...
    public static function consult_ajax()
    {
        ...
        if ($theorder) {
            ...
            // WooCommerce 3.0 or later.
            if (method_exists($theorder, 'get_meta')) {
                $method = $theorder->get_payment_method();
            } else {
                $method = $theorder->payment_method;
            }
