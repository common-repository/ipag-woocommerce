    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields = array(
            ...
            'allow_saved_cards' => array(
                'title' => __('Habilitar pagamentos com cartões salvos'), // __('Enable payments with saved cards', 'ipag-gateway'),
                'label' => __('Enable', 'ipag-gateway'),
                'desc_tip' => __('Quando ativado, os clientes podem optar por salvar os detalhes de pagamento para agilizar compras futuras (obs: os detalhes do cartão não são salvos localmente).'), // __('When enabled, customers can choose to save payment details to speed up future purchases (note: card details are not saved locally).', 'ipag-gateway'),
                'type' => 'checkbox',
                'default' => 'no'
            ),
            'debug' => array(
    ...
    public function savedCards($user_id) {
        ...
        if (class_exists($classTokens)) {
            ...
            foreach (WC_Payment_Tokens::get_customer_tokens($user_id) as $tokenCC) {
                if ($tokenCC->get_gateway_id() == $this->id)
                    $tokens[] = $tokenCC;
            }
    ...
    public function admin_enqueue_scripts($hook_suffix)
    {
        if ($hook_suffix === 'woocommerce_page_wc-settings') {
            ...
            if ($sectionParam === 'ipag-gateway') {
                wp_enqueue_script('ipag-gateway-admin-script', plugins_url('../js/admin-script.js', __FILE__));
            }
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        if ($validPayload || !empty($ipagtoken)) {
            ...
            $payload = array(
                ...
                'gera_token_cartao' => $this->allow_saved_cards() && $this->checkSaveCardRequest(),
                ...
                'retorno_tipo' => 'XML',
                'url_retorno' => self::isLocal() ? "https://ipag-woocommerce.requestcatcher.com/callback?id={$order_id}" : home_url('/?wc-api=wc_gateway_ipag&id=' . $order_id),
            ...
            if ($success) {
                ...
                if ($this->isEmAnalise(get_post_meta($order_id, '_status_payment', false)[0]) && $order->auth_url) {
                    ...
                    self::addDataLog('iniciando autenticação de pagamento (3DS): redirecionando para a url recebida', [
                        ...
                        'url_autenticação' => $order->auth_url
                    ], 'info');
    ...
    public function processCreditCard($payload, $order, $subscription = null)
    {
        ...
        try {
            ...
            if (!empty($payload['gera_token_cartao']) && !empty((string) $xml->token))
            {
                ...
                if ($this->allow_saved_cards() && $this->checkSaveCardRequest())
                {
